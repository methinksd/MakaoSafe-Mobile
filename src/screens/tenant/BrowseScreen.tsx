...
export default function BrowseScreen() {
  ...
  const loadAll = async () => {
    ...
    try {
      ...
      console.warn('[BrowseScreen] loadAll error:', err);
      Toast.show({ type: 'error', text1: 'Could not load properties', text2: 'Check your connection and try again' });
    }
    setLoading(false);
  };
  ...
  const doSearch = useCallback(async (keyword: string) => {
    ...
    try {
      ...
      console.warn('[BrowseScreen] search error:', err);
      Toast.show({ type: 'error', text1: 'Search failed', text2: 'Try again' });
    }
    setSearching(false);
  }, []);
