...
function usePaymentPoller(bookingId: number | null, onPaid: () => void, onFailed: () => void) {
  ...
  useEffect(() => {
    ...
    pollRef.current = setInterval(async () => {
      ...
      // Stop polling after ~90 seconds (18 × 5s)
      if (attempts.current >= 18) clearInterval(pollRef.current!);
    }, 5000);
...
export default function PropertyDetailScreen() {
  ...
  const handleBook = async () => {
    ...
    try {
      ...
      const msg = err.response?.data?.message || err.response?.data || 'Booking failed. Try again.';
      Toast.show({ type: 'error', text1: 'Booking Failed', text2: String(msg) });
    } finally {
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      const msg = err.response?.data?.message || 'STK push failed. Try again.';
